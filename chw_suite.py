...
def all_data_performance_experiment():
    ...
    result_scores = experiment_helper.param_run(feature_data, target_data)
...
def added_features_performance_experiment():
    ...
    x_columns = ['X%d' % i for i in range(1, 91)]
    total_forms = chw_data.dataset[x_columns].sum(axis=1)
    pct_utilized = (chw_data.dataset[x_columns] > 0).sum(axis=1) / 90
    active_last_month = (chw_data.dataset[x_columns[-31:]] > 0).any(axis=1)
    ...
    for feature_name, feature_column in other_features.iteritems():
        ...
        result_scores = experiment_helper.param_run(
    ...
    experiment_helper.write_out_results(
...
def modified_features_experiment():
    ...
    def project_normalize(dataset, feature_set):
        ...
        feature_set = [
            ...
            if not feature.startswith('nX')
        ]
        x_columns = ['X%s' % i for i in range(1, 91)]
        grouped = ds[['projectCode'] + x_columns].groupby('projectCode')
        g_normalized = grouped.apply(
            lambda g: g[x_columns] / math.sqrt(
                (g[x_columns] ** 2).sum(axis=1).sum()
            )
        )
        for column in x_columns:
            ds[column] = g_normalized[column]
        return ds[x_columns + feature_set]
...
def country_to_all_generalization_experiment(inverse=False):
    ...
    region_data_size_min = 500
    countries = [
        key for key, val in chw_data.country.iteritems()
        if val > region_data_size_min
    ]
    out_results = []

    for country in countries:
        column_select = {country: 1}
        feature_data = chw_data.get_features(column_select)
        target_data = chw_data.get_targets(column_select)
        test_features = chw_data.get_features(column_select, exclude=True)
        test_targets = chw_data.get_targets(column_select, exclude=True)
        if not inverse:
            result_scores = experiment_helper.param_run(
...
def sector_to_all_generalization_experiment(inverse=False):
    ...
    for sector in sectors:
        column_select = {sector: 1}
        feature_data = chw_data.get_features(column_select)
        target_data = chw_data.get_targets(column_select)
        test_features = chw_data.get_features(col_filter=column_select,
                                              exclude=True)
        test_targets = chw_data.get_targets(column_select, exclude=True)
        if not inverse:
            result_scores = experiment_helper.param_run(
...
def project_to_all_generalization_experiment(inverse=False):
    ...
    for project_code in project_codes:
        column_select = {'projectCode': project_code}
        feature_data = chw_data.get_features(column_select)
        target_data = chw_data.get_targets(column_select)
        test_features = chw_data.get_features(
            col_filter=column_select, exclude=True
        )
        test_targets = chw_data.get_targets(column_select, exclude=True)
        if not inverse:
            result_scores = experiment_helper.param_run(
...
def project_model_comparison_experiment():
    ...
    for project_code in project_codes:
        ...
        test_features = chw_data.get_features(column_select)
        test_targets = chw_data.get_targets(column_select)
        ...
        all_f = chw_data.get_features(column_select, exclude=True)
        all_t = chw_data.get_targets(column_select, exclude=True)
        ...
        same_country = (
            all_f[all_f[country] == 1],
            all_t[all_f[country] == 1]
        )
        same_sector = (
            all_f[all_f[sector] == 1],
            all_t[all_f[sector] == 1]
        )
        ...
        both_count_too_small = (
            any(same_country[1].value_counts().values < 50)
            or any(same_sector[1].value_counts().values < 50)
        )
                ...
                result_scores = (
                    {'stats': {'label': label},
                     'values': [
                         ...
                         }) for est in experiment_helper.estimators.keys()
                     ]}, {}
                )
                print '\tToo few classes'
            out_results.append(result_scores)
    experiment_helper.write_out_results(
        'combo', out_results, projects, None, 'Accuracy'
    )
...
if __name__ == '__main__':
    ...
    chw_data = ExperimentData(
        'chw_data.csv', label, drop_features,
    ...
    experiment_helper = ExperimentHelper(estimators, args, colours, styles)
    ...
    if args.list:
        ...
        experiment_helper.draw_graph_from_file(
            args.graph, args.split, args.x_ticks
        )
    elif args.graph_folder:
        experiment_helper.find_data_and_draw_graphs(args.graph_folder)
